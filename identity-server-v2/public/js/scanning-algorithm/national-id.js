(function(){
  ...
  const NationalID = {
    ...
    async fillFromText(text) {
      ...
      // Helper: Check if a line is ONLY labels (no actual data)
      const labelWords = new Set(['apelyido','last','name','surname','first','given','names','mga','pangalan','gitnang','middle','date','of','birth','dob','petsa','ng','kapanganakan','sex','kasarian','address','tirahan']);
      const isLabelOnlyLine = (line) => {
        if (!line) return true;
        const words = line.toLowerCase().split(/[\s\/\-:,]+/).filter(Boolean);
        return words.every(w => labelWords.has(w) || w.length < 2);
      };

      // Last Name: Find "Apelyido" or "Last Name" label, then get value from next line if current is label-only
      let lastName;
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const l = line.toLowerCase();
        if (/\b(apelyido|last\s*name|surname)\b/.test(l)) {
          // Check if this line is ONLY labels
          if (isLabelOnlyLine(line)) {
            // Value is on the next line
            if (i + 1 < lines.length && !isLabelOnlyLine(lines[i + 1])) {
              lastName = lines[i + 1].trim();
              break;
            }
          } else {
            // Try to extract from same line
            const after = line.split(/:|\-|–|—/).slice(1).join(':') || line.replace(/.*?(apelyido|last\s*name|surname)\b[:\s\-\/]*/i, '');
            const cleaned = clean(after);
            if (cleaned && !isLabelOnlyLine(cleaned)) {
              lastName = cleaned;
              break;
            }
          }
      ...
      // First Name: Find "Mga Pangalan" or "Given Names" label
      let firstName;
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const l = line.toLowerCase();
        if (/\b(mga\s*pangalan|given\s*names?|first\s*name)\b/.test(l)) {
          if (isLabelOnlyLine(line)) {
            if (i + 1 < lines.length && !isLabelOnlyLine(lines[i + 1])) {
            ...
            const after = line.split(/:|\-|–|—/).slice(1).join(':') || line.replace(/.*?(mga\s*pangalan|given\s*names?|first\s*name)\b[:\s\-\/]*/i, '');
            const cleaned = clean(after);
            if (cleaned && !isLabelOnlyLine(cleaned)) {
              firstName = cleaned;
              break;
      ...
      // Birth Date: Find date of birth label
      let birthDate;
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const l = line.toLowerCase();
        if (/\b(petsa\s*ng\s*kapanganakan|date\s*of\s*birth|dob|birth\s*date)\b/.test(l)) {
          if (isLabelOnlyLine(line)) {
            if (i + 1 < lines.length) {
              const parsed = normalizeDate(lines[i + 1]);
              if (parsed) { birthDate = parsed; break; }
            }
          } else {
            const rx = /(petsa\s*ng\s*kapanganakan|date\s*of\s*birth|dob)[:\s\-\/]*([A-Za-z0-9 ,\/.\-]+)/i;
            const m = line.match(rx);
            if (m && m[2]) {
              const tail = clean(m[2]);
              if (!isLabelOnlyLine(tail)) {
      ...
      // ID Number
      let idNumber;
      // Try to find 16-digit format (####-####-####-####)
      const idMatch = rawText.match(/\b(\d{4}[\-\s]\d{4}[\-\s]\d{4}[\-\s]\d{4})\b/);
      if (idMatch) {
        idNumber = idMatch[1].replace(/\s/g, '');
      }
      if (!idNumber) {
        ...
        if (idx !== -1) {
          ...
          if (num) idNumber = clean(num[1]).replace(/\s+/g, '');
        }
      }
      if (!idNumber) {
