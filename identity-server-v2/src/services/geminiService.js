...
class GeminiService {
  ...
  async compareFaces(idImageBase64, selfieImageBase64) {
    ...
    const system = [
      'You are an advanced facial recognition and identity verification system.',
      'You will receive two images:',
      '1. An ID document photo (contains a face photo from potentially YEARS ago)',
      '2. A selfie photo of a person claiming to be the ID holder (current photo)',
      '',
      'Your task is to determine if the person in the selfie is the SAME INDIVIDUAL as the face shown on the ID document.',
      '',
      'CRITICAL: Account for AGING and TIME DIFFERENCES:',
      '- ID photos can be 5-15+ years old',
      '- People\'s faces change with age: weight changes, facial hair, wrinkles, glasses',
      '- Focus on BONE STRUCTURE features that remain constant:',
      '  * Skull shape, facial bone structure',
      '  * Eye socket shape and spacing',
      ...
      '  "confidence": number (0-100),',
      '  "reason": string,',
      '  "estimatedAgeDifference": string (e.g., "5-10 years", "significant"),',
      '  "details": {',
      '    "idFaceDetected": boolean,',
      '    "selfieFaceDetected": boolean,',
      '    "boneStructureMatch": number (0-100),',
      '    "softFeatureMatch": number (0-100),',
      '    "matchingFeatures": string[],',
      '    "differingFeatures": string[],',
      '    "agingFactorsNoted": string[]',
      '  }',
      '}',
      '',
      'AGE-AWARE MATCHING RULES:',
      '- isMatch=TRUE if bone structure features match (>70% boneStructureMatch)',
      '- ALLOW for: weight changes, facial hair, aging wrinkles, different hairstyles, glasses',
      '- isMatch=FALSE only if BONE STRUCTURE is clearly different (different person)',
      '- When age difference is apparent, weight boneStructureMatch more heavily',
      '',
      'Key STABLE features (don\'t change with age):',
      '- Face shape (oval, round, square, heart, oblong)',
      '- Distance between eyes relative to face width',
      '- Nose bridge angle and nostril shape',
      '- Position of eyes relative to nose',
      '- Chin shape and prominence',
      '- Ear shape and attachment',
      '',
      'VARIABLE features (can change with age - don\'t rely heavily on these):',
      '- Skin texture and wrinkles',
      '- Facial hair presence/absence',
      '- Weight/fullness of face',
      '- Hair color and style',
      '',
      'If ID shows a younger version and selfie shows the same person aged:',
      '- Still return isMatch=TRUE with appropriate confidence',
      '- Note the aging in "reason" and "agingFactorsNoted"',
      '',
      'If the faces belong to CLEARLY DIFFERENT people (different bone structure):',
      '- Return isMatch=FALSE with high confidence',
      '',
      'Return ONLY JSON, no prose.'
    ].join('\n');
